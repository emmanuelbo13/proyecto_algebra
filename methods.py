...
def modulo_27(matriz):
    ...
    for fila in matriz:
        nueva_fila = []
        for elem in fila:
            entero = int(round(elem))   # elimina errores float
            nuevo = entero % 27         # aplica módulo correctamente
...
"""Funcion para convertir una lista en matriz 3xN"""
def lista_a_matriz(lista):
    matriz = []
    for i in range(0, len(lista), 3):
        matriz.append(lista[i:i+3])
    return matriz

"""Funcion para el metodo gauss jordan"""
def gauss_jordan(matriz):
    # Implementación del método de Gauss-Jordan para encontrar la inversa de una matriz
    # matriz = deepcopy(matriz)  # Crear una copia profunda para no modificar la matriz original
    n = len(matriz)
